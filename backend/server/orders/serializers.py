...
class OrderSerializer(serializers.ModelSerializer):
    ...
    provider_mobile = serializers.CharField(source='provider.mobile_number', read_only=True)
    ...
    class Meta:
        model = Orders
        fields = [
            'id', 'user', 'provider', 'provider_name', 'provider_mobile', 'provider_email', 'provider_address',
            'service', 'service_name', 'service_image',
    ...
    def get_provider_address(self, obj):
        if obj.provider:
            parts = [p for p in [obj.provider.street_address, obj.provider.city, obj.provider.state, obj.provider.postal_code] if p]
            return ", ".join(parts) if parts else "Mysore, Karnataka"
