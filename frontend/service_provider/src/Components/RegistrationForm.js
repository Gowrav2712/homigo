...
import {Typography, Alert, Box, Button, TextField, IconButton, InputAdornment, CircularProgress} from "@mui/material";
...
const RegistrationForm = (
  ...
  const handleSubmit = (e) => {
    ...
    if(!formData.main_service) newErrors.main_service = 'Please select a main service';
        ...
        <Grid container rowSpacing={1} columnSpacing={2}>
          ...
          {(error || Object.keys(errors).length > 0) && (
            <Alert severity="error" sx={{mt: 2, width: '100%'}}>
              {error || 'Please fix the errors before submitting the form.'}
            </Alert>
          )}
