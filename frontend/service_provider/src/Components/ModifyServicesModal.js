...
const ModifyServicesModal = ({ open, onClose }) => {
  ...
  const fetchCurrentServices = async () => {
    try {
      ...
      const activeMainServiceId = providerData.main_service?.id || localStorage.getItem('mainServiceId');
      if (activeMainServiceId) {
        localStorage.setItem('mainServiceId', activeMainServiceId);
        await fetchAvailableServices(activeMainServiceId, providerData.provider_services || []);
      }
    } catch(error) {
  ...
  const fetchAvailableServices = async (serviceId, activeProviderServices) => {
    try {
